...
with open('C:\Users\cooper\Documents\data\comp_have_base.fil', 'rb') as bf:
...
try:
    ...
    cur = conn.cursor()
    conn2 = MySQLdb.connect(host='192.168.0.50', user='root', passwd='dingyu', db='dingyu', port=3306,  charset="utf8")
    cur2 = conn2.cursor()
        ...
        if aa:
            ...
            if not (data in f):
                try:
                    ...
                    f.add(data)
                    sql = r"""insert into t_gongsi(t_gongsi_mingzi) value ('%s');""" % (aa[0])
                    cur.execute(sql)
                    conn.commit()
                    f.add(data)
...
with open('C:\Users\cooper\Documents\data\comp_have_base_total_1.fil', 'wb') as abf:
    f.tofile(abf)
