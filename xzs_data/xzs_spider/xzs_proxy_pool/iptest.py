...
def test_ip(ip):
    ...
    try:
        r = requests.get(url, proxies={'http': ip}, timeout=5)
        return {"ip": ip, "speed": r.elapsed.total_seconds()}
    except:
        try:
            r = requests.get(url, proxies={'http': ip}, timeout=5)
            return {"ip": ip, "speed": r.elapsed.total_seconds()}
...
if __name__ == '__main__':
    ...
    # link mongo databases
    conn_mongo = link_mongo(MONGOIP, MONGOPORT)
    db_mongo = conn_mongo.ip
