...
def jiangsu_dist(image, model):
    ...
    return ''.join([chr(int(d1)), chr(int(d2)), chr(int(d3)), chr(int(d4)), chr(int(d5)), chr(int(d6)), ])


def jiangsu_vali(text, model_file=False, show=False):
    if model_file:
        model = model_file
    else:
    ...
    if show:
        image.show()
    return jiangsu_dist(image, model)
...
if __name__ == '__main__':
    ...
    print jiangsu_dist(image, model)
